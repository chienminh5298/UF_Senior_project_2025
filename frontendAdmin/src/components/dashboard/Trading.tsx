        ...
        <div className="h-full flex flex-col p-4 sm:p-6 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 flex-shrink-0 mb-4 sm:mb-6">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                    <Badge className={`text-xs sm:text-sm ${isAutoTrading ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-red-500/20 text-red-400 border-red-500/30"}`}>Auto Trading {isAutoTrading ? "Active" : "Paused"}</Badge>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={isAutoTrading} onChange={(e) => setIsAutoTrading(e.target.checked)} className="sr-only peer" />
                    ...
                    <Card className="bg-gray-900 border-gray-800 flex-shrink-0">
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <select value={selectedToken} onChange={(e) => setSelectedToken(e.target.value)} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500">
                            ...
                            <div className="space-y-3">
                                ...
                                <input type="number" value={orderAmount} onChange={(e) => setOrderAmount(e.target.value)} placeholder="Enter amount..." max={summary?.availableCash || 0} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-blue-500" />
                                <div className="flex justify-between items-center">
                                    <p className="text-sm text-green-400">Maximum investment: ${(summary?.availableCash || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                    <Button type="button" variant="ghost" size="sm" onClick={() => setOrderAmount((summary?.availableCash || 0).toString())} className="text-blue-400 hover:text-white text-xs">
                                        Use Max
                                    </Button>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <Button onClick={handlePlaceOrder} disabled={!selectedToken || !orderAmount || parseFloat(orderAmount) <= 0 || parseFloat(orderAmount) > (summary?.availableCash || 0)} className="w-full bg-green-600 hover:bg-green-700 py-3 text-base font-medium disabled:opacity-50 disabled:cursor-not-allowed">
                                    <Plus className="w-4 h-4 mr-2" />
                                    Place Token Order
                                </Button>
                                {orderAmount && parseFloat(orderAmount) > (summary?.availableCash || 0) && <p className="text-sm text-red-400 text-center">Amount exceeds available cash (${(summary?.availableCash || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})</p>}
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="flex-1 overflow-y-auto min-h-0">
                            <div className="space-y-3">
                                {availableTokens.map((token, index) => {
                                    ...
                                    return (
                                        <div key={`card-${token.id}-${index}`} className={`p-3 rounded-lg border transition-all cursor-pointer ${isSelected ? "bg-blue-500/20 border-blue-500/50 shadow-lg shadow-blue-500/10" : "bg-gray-800/50 border-gray-700 hover:bg-gray-800/70"}`} onClick={() => setSelectedToken(token.name)}>
                                            <div className="flex items-center justify-between mb-2">
                                                <h4 className={`font-medium text-sm ${isSelected ? "text-blue-400" : "text-white"}`}>{token.name}</h4>
                                                <Badge className={`${isSelected ? "bg-blue-500/30 text-blue-300 border-blue-500/50" : "bg-blue-500/20 text-blue-400 border-blue-500/30"}`}>Available</Badge>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 text-xs">
                                                <div>
                                                    <p className="text-gray-400">Min Quantity</p>
                                                    <p className={`font-medium ${isSelected ? "text-blue-300" : "text-white"}`}>{token.minQty || "N/A"}</p>
                                                </div>
                                                <div>
                                                    <p className="text-gray-400">Leverage</p>
                                                    <p className={`font-medium ${isSelected ? "text-blue-300" : "text-white"}`}>{token.leverage || 1}x</p>
                                                </div>
                                            </div>
                                        </div>
                                    );
