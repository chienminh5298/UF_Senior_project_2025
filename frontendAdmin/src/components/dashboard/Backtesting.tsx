...
export function Backtesting() {
    ...
    const { tradesWithCapital, dailySummaries } = useMemo(() => {
        ...
        const summaries = Array.from(tradesByDay.entries())
            .map(([date, dayTrades]) => {
    ...
    const finalCapital = tradesWithCapital.length > 0 ? tradesWithCapital[tradesWithCapital.length - 1].capitalAfter : initialCapital;
    ...
    return (
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <Card className="bg-gray-900 border-gray-800">
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <select value={selectedStrategy} onChange={(e) => setSelectedStrategy(e.target.value)} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500" disabled={isRunning || isAnimating || strategies.length === 0}>
                                {strategies.length === 0 ? (
                            ...
                            <div className="min-h-[2.5rem]">{selectedStrategy && strategies.find((s) => s.id === selectedStrategy) && <p className="text-xs text-gray-400 mt-1">{strategies.find((s) => s.id === selectedStrategy)?.description}</p>}</div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                {years.map((year) => (
                                    <button key={year} onClick={() => setSelectedYear(year)} disabled={isRunning || isAnimating} className={`p-2 rounded text-sm font-medium transition-colors ${selectedYear === year ? "bg-blue-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"} disabled:opacity-50 disabled:cursor-not-allowed`}>
                                        {year}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <input type="number" value={initialCapital} onChange={(e) => setInitialCapital(Number(e.target.value))} className="w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-blue-500" min="100" max="1000000" disabled={isRunning || isAnimating} />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-5 gap-2">
                                {renderSpeeds.map((speed) => (
                                    <button key={speed.value} onClick={() => setRenderSpeed(speed.value)} disabled={isRunning || isAnimating} className={`p-2 rounded text-sm font-medium transition-colors ${renderSpeed === speed.value ? "bg-green-600 text-white" : "bg-gray-800 text-gray-300 hover:bg-gray-700"} disabled:opacity-50 disabled:cursor-not-allowed`}>
                                        {speed.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* Run Button */}
                        {!isAnimating ? (
                            <Button onClick={handleRunBacktest} disabled={isRunning || !selectedStrategy} className="w-full bg-blue-600 hover:bg-blue-700 py-3">
                                {isRunning ? (
                                    <>
                            ...
                            <Button onClick={handleStopAnimation} className="w-full bg-red-600 hover:bg-red-700 py-3">
                                Stop Backtest
                            </Button>
                        )}
                    </CardContent>
            ...
            {hasRun && backtestResults && backtestResults.trades && backtestResults.trades.length > 0 && <DailyTradeSummary dailySummaries={dailySummaries} totalTrades={tradesWithCapital.length} currentPage={currentPage} tradesPerPage={tradesPerPage} selectedDate={selectedDate} onSelectDate={setSelectedDate} onPageChange={setCurrentPage} />}

            {/* Performance Metrics - Only show after animation completes */}
            {hasRun && backtestResults && !isAnimating && <PerformanceMetrics results={backtestResults} token={selectedToken} strategy={strategies.find((s) => s.id === selectedStrategy)?.name || selectedStrategy} year={selectedYear} initialCapital={initialCapital} finalCapital={finalCapital} />}
        </div>
    );
}
