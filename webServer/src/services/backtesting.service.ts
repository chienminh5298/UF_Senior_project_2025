...
export class BacktestingService {
  private static readonly GOOGLE_SCRIPT_URL =
    'https://script.google.com/macros/s/AKfycbwgM2khVjBMvedIXx4S2GuUKvl7LoWUUhkSP0OsaCaf-HiU2dAxBRhCXk_eyHW5tXd1_Q/exec';
  private static readonly HISTORICAL_DATA_DIR = path.join(
    process.cwd(),
    'historical_data'
  );
  ...
  private async loadLocalData(
    ...
    const filePatterns = [
      path.join(
        BacktestingService.HISTORICAL_DATA_DIR,
        token,
        `${token}${year}.json`
      ),
    ];
    ...
    if (!fileExists) {
      console.log(
        `📁 Local file not found. Tried: ${token}/${year}.json and ${token}/${token}${year}.json`
      );
      return [];
    }
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        console.log(
          `📋 Converted object format to array (${dataArray.length} entries)`
        );
      } else {
        console.error(
          `❌ Invalid local data format for ${path.basename(filePath)} - expected array or object`
        );
        return [];
      }
      ...
      const candles: CandleData[] = dataArray.map((item: any) => ({
        timestamp: new Date(
          item.timestamp || item.time || item.date || item.Date
        ).getTime(),
      ...
      console.log(
        `✅ Loaded ${candles.length} candles from local file: ${path.basename(filePath)}`
      );
      return candles;
    } catch (error) {
      console.error(
        `❌ Error loading local file ${path.basename(filePath)}:`,
        error
      );
      return [];
    }
  ...
  private async fetchFromGoogleScript(
    ...
    try {
      ...
      if (!response.data || !Array.isArray(response.data)) {
        console.error(
          `❌ Invalid response from Google Script for ${token} ${year}:`,
          response.data
        );
        throw new Error(
          `Invalid response from Google Script: expected array, got ${typeof response.data}`
        );
      }

      if (response.data.length === 0) {
        console.warn(
          `⚠️  Google Script returned empty array for ${token} ${year}`
        );
        return [];
      }
      ...
      console.log(
        `✅ Fetched ${candles.length} candles from Google Script for ${token} ${year}`
      );
      return candles;
    } catch (error) {
      console.error(
        `❌ Failed to fetch data from Google Script for ${token} ${year}:`,
        error
      );
  ...
  async fetchHistoricalData(
    ...
    console.log(
      `🔍 Fetching data for ${symbol} (${new Date(startTime).toISOString()} to ${new Date(endTime).toISOString()})`
    );
    ...
    try {
      ...
      if (supportedTokens.includes(token)) {
        ...
        for (
          let year = Math.max(2023, startYear);
          year <= Math.min(2025, endYear);
          year++
        ) {
          years.push(year);
        }
        ...
        for (const year of years) {
          try {
            ...
            fetchErrors++;
            console.warn(
              `⚠️  Failed to load ${token} data for year ${year}:`,
              error
            );
          }
        }
        ...
        if (allCandles.length > 0) {
          ...
          console.log(
            `✅ Successfully fetched ${filtered.length} candles from local files for ${symbol}`
          );
          console.log(
            `   Total candles before filtering: ${allCandles.length}`
          );
          if (filtered.length > 0) {
            console.log(
              `   Date range: ${new Date(filtered[0]?.timestamp).toISOString()} to ${new Date(filtered[filtered.length - 1]?.timestamp).toISOString()}`
            );
          }
          return filtered;
        } else {
          ...
          console.error(
            `   Errors encountered: ${fetchErrors}/${years.length}`
          );
          console.error(
            `   Make sure files exist: historical_data/${token}/${token}2024.json or historical_data/${token}/2024.json`
          );
          throw new Error(
            `No local data files found for ${token}. Please ensure data files exist in historical_data/${token}/`
          );
        }
      } else {
        console.log(
          `⚠️  Token ${token} not supported. Supported: ${supportedTokens.join(', ')}`
        );
      ...
      throw new Error(
        `Failed to load historical data: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async fetchLiveData(
    ...
    try {
      console.log(
        `Falling back to live API for ${symbol}, interval: ${interval}, from: ${new Date(startTime).toISOString()} to: ${new Date(endTime).toISOString()}`
      );
      ...
      throw new Error(
        'Failed to fetch historical market data from Binance API'
      );
    }
  }
  ...
  private executeMomentumStrategy(
    ...
    for (let i = longPeriod; i < data.length; i++) {
      ...
      const shortMA = this.calculateSMA(
        data.slice(i - shortPeriod, i),
        shortPeriod
      );
      const longMA = this.calculateSMA(
        data.slice(i - longPeriod, i),
        longPeriod
      );
      const prevShortMA = this.calculateSMA(
        data.slice(i - shortPeriod - 1, i - 1),
        shortPeriod
      );
      const prevLongMA = this.calculateSMA(
        data.slice(i - longPeriod - 1, i - 1),
        longPeriod
      );
      ...
      if (position === 'LONG' && (shortMA < longMA || i === data.length - 1)) {
        const pnl =
          ((currentPrice - entryPrice) / entryPrice) * config.initialCapital;
        trades.push({
          id: `trade_${tradeId++}`,
          timestamp: data[i].timestamp,
          side: 'SELL',
        ...
        position === 'SHORT' &&
        (shortMA > longMA || i === data.length - 1)
      ) {
        const pnl =
          ((entryPrice - currentPrice) / entryPrice) * config.initialCapital;
        trades.push({
          id: `trade_${tradeId++}`,
          timestamp: data[i].timestamp,
          side: 'BUY',
   ...
   */
  private executeScalpingStrategy(
    data: CandleData[],
    config: BacktestConfig
  ): Trade[] {
    const trades: Trade[] = [];
  ...
  private calculateMetrics(
    ...
    const holdTimes = completeTrades.map(
      (t) => (t.exit.timestamp - t.entry.timestamp) / (1000 * 60 * 60)
    );
    const avgHoldTime =
      holdTimes.length > 0
    ...
    return {
      totalReturn,
      totalReturnPercent: totalReturnPercent.startsWith('-')
        ? totalReturnPercent
        : `+${totalReturnPercent}%`,
      ...
      avgTrade,
      avgTradePercent: avgTradePercent.startsWith('-')
        ? avgTradePercent
        : `+${avgTradePercent}%`,
      bestTrade,
  ...
  async executeBacktest(config: BacktestConfig): Promise<BacktestResults> {
    try {
      ...
      console.log(
        `Executing backtest with ${historicalData.length} candles from ${new Date(historicalData[0].timestamp).toISOString()} to ${new Date(historicalData[historicalData.length - 1].timestamp).toISOString()}`
      );
      ...
      console.log(
        `Generated ${trades.length} trades from ${new Date(trades[0]?.timestamp || 0).toISOString()} to ${new Date(trades[trades.length - 1]?.timestamp || 0).toISOString()}`
      );
